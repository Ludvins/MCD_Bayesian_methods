...
def load_mnist(sklearn = False):
    ...
    if sklearn:
        ...
        train_images, test_images = np.array(X[:60000]), np.array(X[60000:])
        train_labels, test_labels = np.array(y[:60000]), np.array(y[60000:])
    else:
...
def save_images(images, filename = None, **kwargs):
    fig = plt.figure(1)
    fig.clf()
